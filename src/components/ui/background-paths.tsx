...
// Import changed from "framer-motion" → "motion/react" (rebranded package).
// Uses our existing shadcn v4 Button (base-ui under the hood) — same `variant="ghost"` API.
        ...
        >
          ...
          {subtitle && (
            <motion.p
              ...
              className="text-base sm:text-lg md:text-xl text-neutral-600 dark:text-neutral-400 mb-8 max-w-2xl mx-auto leading-relaxed"
            >
              {subtitle}
            </motion.p>
          )}
          ...
          <div className="inline-block group relative bg-gradient-to-b from-black/10 to-white/10 dark:from-white/10 dark:to-black/10 p-px rounded-2xl backdrop-blur-lg overflow-hidden shadow-lg hover:shadow-xl transition-shadow duration-300">
            {buttonHref ? <Link href={buttonHref}>{ctaContent}</Link> : ctaContent}
          </div>
        </motion.div>
      </div>
    </div>
